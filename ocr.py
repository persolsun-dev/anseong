...
@st.cache_resource
def get_ocr_reader():
    # 한국어 및 영어 인식용 OCR 리더 초기화 (CPU 모드)
...
# 카메라 입력 위젯 (모바일 브라우저에서 카메라 즉시 실행)
...
if target_image is not None:
    ...
    with st.spinner("AI가 청구기호를 인식하는 중입니다..."):
        ...
        # OCR 수행 (상단에서 하단 순서 또는 좌측에서 우측 순서로 정렬 필요)
        ocr_results = reader.readtext(img)

        # 인식된 텍스트 수집 (Y좌표 기준 정렬로 위에서 아래 순서 보장)
        # ocr_results 형태: [([x1,y1], [x2,y2], ...), text, confidence]
    ...
    # 텍스트 에디터로 사용자가 직접 검토/수정할 수 있도록 제공
